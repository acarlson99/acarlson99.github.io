...
#+BEGIN_SRC glsl :file ./static/shadergen/peripheral-drift.png :width 1000 :height 1000 :exports both
  ...
  void main() {
      ...
      float smallCircle = 1.- smoothstep(.3,.3+blur,d);
      float bigCircle = 1.-smoothstep(.4,.4+blur,d);
      vec3 outlineCol = mix(vec3(0.),vec3(1.), bigCircle);
      uvz *= PI*2.;
      ...
      if ((rot(wav)*p).y<0.)
          outlineCol *= .0;
      col = mix(outlineCol, vec3(0.9,.8,.9), smallCircle);
      col = mix(vec3(.5), col, bigCircle);

...
#+NAME: sin
#+BEGIN_SRC glsl :file ./static/shadergen/sin.png :width (/ 1200 2) :height (/ 1200 (* 2 6)) :exports both
  void main() {
...
#+NAME: sin-gradient
#+BEGIN_SRC glsl :file ./static/shadergen/sin-gradient.png :width (/ 1200 2) :height (/ 1200 (* 2 6)) :exports both
  #define PI 3.14159
...
#+NAME: square
#+BEGIN_SRC glsl :file ./static/shadergen/square.png :width (/ 1200 2) :height (/ 1200 (* 2 6)) :exports both
  void main() {
      vec2 uv = (gl_FragCoord.xy - .5*iResolution.xy) / iResolution.xy;
...
#+BEGIN_SRC glsl :file ./static/shadergen/square-gradient.png :width (/ 1200 2) :height (/ 1200 (* 2 6)) :exports both
  #define PI 355./113.
  void main() {
      vec2 uv = (gl_FragCoord.xy - .5*iResolution.xy) / iResolution.xy;

  #define sign(a) (vec3(sign(a[0]),sign(a[1]),sign(a[2])))
      // really this is just `sign(sin(x))`
      vec3 col = sign(sin(
                          vec3(0.,1.,2.)*2.*PI/3.
                          +(uv.x*3.14*2.)*3.));
      fragColor = vec4(.5+.5*col, 1.);
  }
#+END_SRC
...
#+NAME: triangle
#+BEGIN_SRC glsl :file ./static/shadergen/triangle.png :width (/ 1200 2) :height (/ 1200 (* 2 6)) :exports both
  #define PI 3.14159
...
#+BEGIN_SRC glsl :file ./static/shadergen/triangle-gradient.png :width (/ 1200 2) :height (/ 1200 (* 2 6)) :exports both
  #define PI 3.14159
  ...
  void main() {
      ...
      vec3 col = ((2.*amp)/PI)* asin(sin(
                                         vec3(0.,1.,2.)*2.*PI/3.
                                         +(2.*PI*th - phase)/wavelength));
      fragColor = vec4(.5*.5+col, 1.);
  }
#+END_SRC
...
#+NAME: sawtooth
#+BEGIN_SRC glsl :file ./static/shadergen/sawtooth.png :width (/ 1200 2) :height (/ 1200 (* 2 6)) :exports both
  #define PI 3.14159
...
#+BEGIN_SRC glsl :file ./static/shadergen/sawtooth-gradient.png :width (/ 1200 2) :height (/ 1200 (* 2 6)) :exports both
  #define PI 3.14159
  ...
  void main() {
      ...
      vec3 col = (2.*amp/PI) * atan(tan(
                                         vec3(0.,1.,2.)*PI*2./3.
                                         +(th*2.*PI - phase) / (2.*wavelength)));
...
Here we can observe the effect of the log polar transform on the standard `[-.5 .. .5]` coordinate system.

Notice the roughly 6.2 rotations, and spiraling effect produced from the diagonal lines in UV space.

Polar conversion functions are the same-- simply remove the `log` and `exp` function calls

#+NAME: log-polar-coord
#+BEGIN_SRC glsl :file ./static/shadergen/log-polar-coord.png :width 1200 :height (/ 1200 2) :exports both
  void main() {
      ...
      if (lhs)
          p = .5+(
              ...
              vec2(cos(p.x),sin(p.x))*(exp(p.y))
              );

      vec3 c = p.xyx;
      c = mix(p.xyx, p.yyx, step(0.,sin(6.*(p.x + p.y))));
      fragColor = vec4(fract(c), 1.);
  }
#+END_SRC

#+RESULTS: log-polar-coord
[[file:./static/shadergen/log-polar-coord.png]]

** Hyperbolic Mobuis

see https://en.wikipedia.org/wiki/M%C3%B6bius_transformation

#+NAME: hyperbolic-mobius
#+BEGIN_SRC glsl :file ./static/shadergen/hyperbolic-mobius.png :width 1200 :height (/ 1200 2) :exports both
  vec2 cm(float a, float b, float c, float d) {
      return vec2(
          a*c - b*d,
          a*d + b*c
          );
  ...
  vec2 mobius(vec2 z,
              float a,float b,
              float c,float d) {
  //      vec2 num = vec2(a*z.x + b,
  //                      a*z.y);
      vec2 num = cm(z[0],z[1],a,b) + vec2(b,0.);
	
      // vec2 den = vec2(c*z.x + d,
      //                 c*z.y);
      vec2 den = cm(z[0],z[1],c,d) + vec2(d,0.);
	
      float inv = 1.0/dot(den,den);
	
      return vec2(
          (num.x*den.x + num.y*den.y)*inv,
  ...
  void main() {
      ...
      // hand-selected nice looking parameters
      vec2 p = mobius(uv, 0.5,1.,1.,0.);

      if (lhs) p = uv;

      vec3 c = p.xyx;
      c = mix(p.xyx, p.yyx, step(0.,sin(6.28*(p.x + p.y))*sin(6.28*(p.x - p.y))));
      fragColor = vec4(fract(c), 1.);
  }
#+END_SRC

#+RESULTS: hyperbolic-mobius
[[file:./static/shadergen/hyperbolic-mobius.png]]


* low-resolution patterns (like for light arrays)

** Motion (three is all you need)
...
#+BEGIN_SRC glsl :file /dev/null :exports code :noweb no-export :noweb-prefix no
  ...
  void main() {
      ...
      vec2 grid = floor((uv + 0.5) * vec2(6.0,3.)); // 6x6 repeating grid
      float pattern = 1.-mod(grid.x + grid.y, N);
      fragColor = vec4(vec3(pattern), 1.0);
  }
#+END_SRC

Here we define a 6x6 grid, with every second cell turned on.  This creates a checkerboard pattern.

#+NAME: checkerboard-2
#+BEGIN_SRC glsl :file ./static/shadergen/checkerboard-2.png :width (* 6 4) :height (* 3 4) :exports both :noweb no-export :noweb-prefix no
  #define N 2.0
  // <<checkerboard-1>>
#+END_SRC

#+RESULTS: checkerboard-2
[[file:./static/shadergen/checkerboard-2.png]]

By adding a second dark cell (i.e. every third cell is turned on) we can create a motion effect.
TODO: animate this perchance

#+NAME: checkerboard-3
#+BEGIN_SRC glsl :file ./static/shadergen/checkerboard-3.png :width (* 6 4) :height (* 3 4) :exports both :noweb no-export :noweb-prefix no
  #define N 3.0
  // <<checkerboard-1>>
#+END_SRC

#+RESULTS: checkerboard-3
[[file:./static/shadergen/checkerboard-3.png]]
...
Generally this is used at the end of a color processing pipeline to
fit perceptual brightness.
...
#+BEGIN_SRC glsl :file /dev/null :exports code
  ...
  #endif // RGB_CONVERSION_FNS
#+END_SRC

**** Example

#+NAME: srgb-colorspace-demo
#+BEGIN_SRC glsl :file ./static/shadergen/srgb-colorspace-demo.png :width 256 :height 256 :exports both :noweb no-export :noweb-prefix no
  // <<srgb-conversion-fns>>
  // <<hsv-conversion-fns>>
  // <<oklch-conversion-fns>>

  float sum3(vec3 p){return p.x+p.y+p.z;}
  ...
  void main() {
      ...
      float lightness = sum3(col*vec3(0.299, 0.587, 0.114));
...
#+RESULTS: srgb-colorspace-demo
[[file:./static/shadergen/srgb-colorspace-demo.png]]

Comparing linear RGB (top) to the gamma-corrected sRGB (bottom) gradient we can observe a more visually consistent lightness.  Notice how the sharp peaks around yellow, cyan, and violet are smoothed out after converting to SRGB, while the dip in perceptual lightness centered around blue becomes a much steeper drop.
...
#+BEGIN_SRC glsl :file ./static/shadergen/hsv-colorspace-demo.png :width 256 :height 256 :exports both :noweb no-export :noweb-prefix no
  // <<hsv-conversion-fns>>
  // <<srgb-conversion-fns>>
  ...
  void main() {
      ...
      vec3 hsv1 = vec3(
          ...
          1.-smoothstep(.5,1.,y) // fade out to black above
          );
      // bottom color
      vec3 hsv2 = vec3(
  ...
  #define EFFECT1(C) C = mix(						\
          hsv2linear(linear2hsv(C)+vec3(-.05,0.1,0.)),	\
          C,										\
          smoothstep(-blur,blur,wav));
  // desaturate slightly
  #define EFFECT2(C) C = mix(						\
          hsv2linear(linear2hsv(C)*vec3(1.,.7,.4)),		\
          C,										\
          smoothstep(-blur,blur,wav));
      ...
      col = linear2srgb( mix(c2,c1,smoothstep(.2,.25,y)) );

      fragColor = vec4(col,1.0);
  }
#+END_SRC

#+RESULTS: hsv-colorspace-demo
[[file:./static/shadergen/hsv-colorspace-demo.png]]
...
#+BEGIN_SRC glsl :file /dev/null :exports code :noweb no-export :noweb-prefix no
  ...
  #endif // OKLAB_CONVERSION_FNS
#+END_SRC

**** Example

Given the above devinitions we can do some lightness equalization on a gradient
using [[https://stackoverflow.com/questions/596216/formula-to-determine-perceived-brightness-of-rgb-color][this]] [[https://www.merriam-webster.com/thesaurus/timeless][imperishable]] stackoverflow post.

#+NAME: oklab-demo
#+BEGIN_SRC glsl :file ./static/shadergen/oklab-colorspace-demo.png :width 256 :height 256 :exports both :noweb no-export :noweb-prefix no
  ...
  float sum3(vec3 p){return p.x+p.y+p.z;}
    ...
    void main() {
        ...
        if (abs(uv.y)<0.25) {
            float lightness = sum3(c*vec3(0.299, 0.587, 0.114));
...
#+RESULTS: oklab-demo
[[file:./static/shadergen/oklab-colorspace-demo.png]]

*** oklch

Lightness in range [0 .. 1] (as chroma increases this begins to clip the gamut at L=1)
...
For a geometric understanding I invite the reader to play around with oklch.com
...
#+BEGIN_SRC glsl :file /dev/null :exports code :noweb no-export :noweb-prefix no
  ...
  #endif // OKLCH_CONVERSION_FNS
#+END_SRC

**** Example

#+NAME: oklch-demo
#+BEGIN_SRC glsl :file ./static/shadergen/oklch-colorspace-demo.png :width 256 :height 256 :exports both :noweb no-export :noweb-prefix no
  ...
  void main() {
      ...
      fragColor = vec4(linear2srgb(c), 1.);
  }
#+END_SRC

#+RESULTS: oklch-demo
[[file:./static/shadergen/oklch-colorspace-demo.png]]
...
#+NAME: color-mix
#+BEGIN_SRC glsl :file ./static/shadergen/color-mix-shader-out.png :width (/ 1080 2) :height (/ 1080 8) :exports both :noweb no-export :noweb-prefix no
  // <<hsv-conversion-fns>>
  // <<srgb-conversion-fns>>
  // <<oklab-conversion-fns>>
  // <<oklch-conversion-fns>>
...
#+RESULTS: color-mix
[[file:./static/shadergen/color-mix-shader-out.png]]
...
#+BEGIN_SRC glsl :file ./static/shadergen/color-mix-shader-out-skibidi.png :width (/ 1080 2) :height (/ 1080 8) :exports results :noweb no-export :noweb-prefix no
  #define MODE 1
  // <<color-mix>>
#+END_SRC

#+RESULTS: color-mix-skibidi
[[file:./static/shadergen/color-mix-shader-out-skibidi.png]]
...
This page does not support animated shaders quite yet-- in the meantime check out my [[./static/widgets/vision-go-blleeehhhhh.html][motion aftereffect induction engine]]
...
#+BEGIN_SRC glsl :file ./static/shadergen/peripheral-drift.png :width 1000 :height 1000 :exports both
  ...
  void main() {
      ...
      vec3 outline1 = vec3(0.);
      vec3 outline2 = vec3(1.);
      vec3 circleColor = vec3(.9,.8,.9);
      vec3 bgColor = vec3(.5);

  #define RGB(r,g,b) (vec3(r,g,b)/256.)
      vec3 oxidizediron = RGB(155, 41, 21);
      outline1 = RGB(4, 31, 30);
      outline1 = RGB(12, 18, 12);
      outline2 = RGB(247, 219, 167);
      circleColor = RGB(239, 121, 138);
      bgColor = RGB(80, 162, 167);

      circleColor = oxidizediron;

      vec2 p = fract(uvz*sc)-.5;
      ...
      float radius = .3;
      float thickness = .075;
      float smallCircle = 1.- smoothstep(radius,radius+blur,d);
      float bigCircle = 1.-smoothstep(radius+thickness,radius+thickness+blur,d);
      uvz *= PI*2.;
      float wav = atan(uvz.x,uvz.y)*3.
          +cos(uvz.x*2.)
          +sin(uvz.y*3.);
      vec3 outlineCol = mix(outline2,outline1,step((rot(wav)*p).y,0.));
      col = mix(outlineCol, circleColor, smallCircle);
      col = mix(bgColor, col, bigCircle);

      //   col = vec3(outlineCol);
      // col *= .5+.5*vec3(sin(wav));
      // col *= uv.xxy;
...
#+BEGIN_SRC glsl :file ./static/shadergen/peripheral-drift-3.png :width 1000 :height 1000 :exports both :noweb no-export :noweb-prefix no
  // <<srgb-conversion-fns>>
  #define PI (355./113.)
  ...
  void main()
  {
      ...
      // inverse srgb conversion for a more intense illusion
      fragColor = vec4(srgb2linear(col),1.0);
  }
#+END_SRC
...
#+NAME: pdrift-sin-1
#+BEGIN_SRC glsl :file ./static/shadergen/pdrift-sin-1.png :width 1080 :height 566 :exports both :noweb no-export :noweb-prefix no
  // <<srgb-conversion-fns>>
  void main() {
      ...
      fragColor = vec4(linear2srgb(.5+.5*c), 1.);
  }
#+END_SRC

#+RESULTS: pdrift-sin-1
[[file:./static/shadergen/pdrift-sin-1.png]]

#+NAME: pdrift-sin-2
#+BEGIN_SRC glsl :file ./static/shadergen/pdrift-sin-2.png :width 1080 :height 566 :exports both :noweb no-export :noweb-prefix no
  // <<srgb-conversion-fns>>
  void main() {
      vec2 r = vec2(.5,.525);
...
#+RESULTS: pdrift-sin-2
[[file:./static/shadergen/pdrift-sin-2.png]]

#+NAME: pdrift-sin-3
#+BEGIN_SRC glsl :file ./static/shadergen/pdrift-sin-3.png :width 1080 :height 566 :exports both :noweb no-export :noweb-prefix no
  // <<srgb-conversion-fns>>
  void main() {
      vec2 uv = (gl_FragCoord.xy - vec2(.5,.55)*iResolution.xy) / iResolution.xy;
      float th = abs(uv.x)*2.;
...
#+RESULTS: pdrift-sin-3
[[file:./static/shadergen/pdrift-sin-3.png]]

**** lch hsv gradient swirls

#+NAME: color-mix-cmp-1
#+BEGIN_SRC glsl :file ./static/shadergen/color-mix-shader-out-whaa.png :width 2048 :height 2048 :exports both :noweb no-export :noweb-prefix no
  // <<hsv-conversion-fns>>
  // <<oklch-conversion-fns>>
  // <<srgb-conversion-fns>>
  ...
  void main() {
      ...
      vec3 C = vec3(0.);
      int AAAmt = 1;
      for (int ox=-AAAmt;ox<=AAAmt;ox++){
          for (int oy=-AAAmt;oy<=AAAmt;oy++){
              samples += 1.;

              vec2 uv = gl_FragCoord.xy+vec2(ox,oy) / iResolution.xy;
              vec2 uvz = (gl_FragCoord.xy+vec2(ox,oy) - .5*iResolution.xy)
                  /iResolution.y;

              vec3 col = vec3(1.,1.,1.);

              float a = atan(uvz.y,uvz.x);
              float th = a*6.+sin(length(uvz)*PI*6.*2.*2.)*PI+length(uvz)*PI*2.*6.;
              float wav = sin(th);
              float wav2 = fract(th/PI/2.);

              vec3 lch = vec3(1.,
                              mix(wav2,length(uvz),.25),
                              atan(uvz.y,uvz.x)*3.+wav+length(uvz)*PI*3.);
              vec3 ca = oklch2linear(lch*vec3(.8,.3,1.));
      ...
      vec3 col = vec3(C)/samples;

      // inverse srgb conversion to intensify
      // light-dark contrast and strengthen illusion
      fragColor = vec4(srgb2linear(col),1.0);
  }
#+END_SRC
...
Not even sure with this one, but the tiles kinda make crosses in different directions

#+NAME: illusion-perspective
#+BEGIN_SRC glsl :file ./static/shadergen/illusion-perspective.png :width 1024 :height 1024 :exports both
  #define PI 335./113.
  ...
  void main() {
      ...
      vec3 col = vec3(1.);
      // sin(x)+sin(y) for checkers
      col *= step(0.,cos(uv.x*PI*20.+uv.y*PI*0.)*cos(uv.y*PI*10.+uv.x*PI*10.));
      col *= mix(
          mix(1.,.25,.5+uv.x),
          mix(.25,1.,.5+uv.x),
          step(0.,uv.y));
